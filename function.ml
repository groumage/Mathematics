 ...
 ****************************************)
let string_of_float_enh f = if Util.is_float_int f then string_of_int (int_of_float f) else string_of_float f
...
let raise_power_mono m n = (Util.power (get_coeff m) n,
...
let gcd_powerlist pl1 pl2 =
  let rec gcd_powerlist_helper pl1 pl2 intvars so_far =
    ...
    | x::xs ->
      ...
      if Util.is_float_int p1 && Util.is_float_int p2 then
        gcd_powerlist_helper pl1 pl2 xs (so_far @ [(x, min p1 p2)])
      else
        gcd_powerlist_helper pl1 pl2 xs so_far

  in gcd_powerlist_helper pl1 pl2 (Util.intersect_list (List.map get_var pl1) (List.map get_var pl2)) [];;
...
let gcd_mono m1 m2 =
  ...
  let gcd_coeff =
    if Util.is_float_int f1 && Util.is_float_int f2 then
      float_of_int (Util.gcd (int_of_float f1) (int_of_float f2))
    else
      1. in
...
let is_poly_float p = List.length p = 1 && is_mono_float (Util.get_first_of_list p);;
...
let float_of_poly p = if is_poly_float p then float_of_mono (Util.get_first_of_list p) else failwith ("Cannot convert polynomial " ^ string_of_poly p ^ " to float");;
...
let simplify_poly p =
  if List.length p = 0 then        (* All terms eliminated, put in constant zero *)
    ...
    let last = Util.get_last_of_list p in
...
let gcd_poly p1 p2 =
  let ml = p1 @ p2 in
  List.fold_left gcd_mono (Util.get_first_of_list ml) (Util.get_rest_of_list ml);;
...
let rec simplify_expr e =
  match e with
    Leaf t ->
      (match t with
        Frac (n, d) ->
          (* p = n * first_of_list(d) ^ (-1) *)
          let p = mult_poly n (poly_of_mono (invert_mono (Util.get_first_of_list d))) in
          (* the power list of  *)
          ...
          (* a list of boolens is created by map and analysed by fold_right
          	 each boolean indicate if the polynomial power at this position is positive or not*)
          if List.length d = 1 && (List.fold_left (&&) true (List.map (fun p -> get_raisedto p >= 0.) powerlists)) then
            Leaf(Poly(p))

          else   (* Can't eliminate denominator, but we'll cancel out common terms by doing divisions *)
  ...
  | Node (o, el) ->
  	  (* We simplify the rest of the expr in the node *)
      ...
      (match o with
        Plus ->
          if List.length sel < 2 then
            ...
            let simp_expr = if List.length new_poly_frac_expr = 1 && is_term_float (term_of_expr (Util.get_first_of_list new_poly_frac_expr)) &&
                              float_of_term (term_of_expr (Util.get_first_of_list new_poly_frac_expr)) = 0. then [] else new_poly_frac_expr in
            ...
            if List.length repl_exprs > 1 then
              ...
              Util.get_first_of_list repl_exprs
            else
      ...
      | Minus ->
          if List.length sel != 2 then
            ...
            let first  = Util.get_first_of_list sel in
            let second = Util.get_first_of_list (Util.get_rest_of_list sel) in
      ...
      | Times ->
          if List.length sel < 2 then
            ...
            if List.length new_poly_frac_expr = 1 && is_term_float (term_of_expr (Util.get_first_of_list new_poly_frac_expr)) &&
              float_of_term (term_of_expr (Util.get_first_of_list new_poly_frac_expr)) = 0. then
              Leaf(Poly(poly_of_float 0.))   (* 0x = 0 *)

            else
              (* If new term evaluated to one, we prepare in case we should eliminate this extraneous term *)
              let simp_expr = if List.length new_poly_frac_expr = 1 && is_term_float (term_of_expr (Util.get_first_of_list new_poly_frac_expr)) &&
                                float_of_term (term_of_expr (Util.get_first_of_list new_poly_frac_expr)) = 1. then [] else new_poly_frac_expr in
          ...
          if List.length sel != 2 then
            ...
            if second_float && float_of_term (term_of_expr second) = 0. then
              ...
              if List.length first_poly = 1 && second_float then   (* Monomial raised to constant power is monomial *)
                Leaf(Poly(poly_of_mono (raise_power_mono (Util.get_first_of_list first_poly) (float_of_poly second_poly))))
              else
              ...
              let base = Util.get_first_of_list operands in
              let exponent = Util.get_first_of_list (Util.get_rest_of_list operands) in
...
let contains_poly p v =
  let m = Util.get_first_of_list p in
  let pl = get_powerlist m in
  let vars = List.map get_var pl in
  let intvars = Util.intersect_list vars [v] in
  ...
  if contains_expr right v then
    ...
    | Node (o, el) ->
        ...
        let rewritten =
          ...
          | Minus ->
              let first  = Util.get_first_of_list el in
              let second = Util.get_first_of_list (Util.get_rest_of_list el) in
    ...
    if is_term_poly t then
      ...
      if List.length p = 1 then
        ...
        if c = 1. && List.length pl = 1 then
          let p        = Util.get_first_of_list pl in
 ...
 ****************************************)
let rec eval_powerlist pl x =
  match pl with
    [] -> 1.
  | h :: t -> Util.power x (get_raisedto h) *. eval_powerlist t x

let rec eval_mono m x = get_coeff m *. eval_powerlist (get_powerlist m) x

let rec eval_poly p x =
  match p with
    [] -> 0.
  | h :: t -> eval_mono h x +. eval_poly t x

let rec eval_frac f x = eval_poly (get_numer f) x /. eval_poly (get_denom f) x

let rec eval_expr e x =
  match e with
    Leaf t ->
      (match t with
        Poly p -> eval_poly p x
      | Frac (n, d) -> eval_poly n x /. eval_poly d x
      | Exp (f1, f2) -> Util.power (eval_frac f1 x) (eval_frac f2 x)
      )
  | Node (o, l) ->
      (match o with
        Plus -> eval_list l (fun a b -> a +. b) x
      | Minus -> eval_list l (fun a b -> a -. b) x
      | Times -> eval_list l (fun a b -> a *. b) x
      | Div -> eval_list l (fun a b -> a /. b) x
      | Power -> eval_list l (fun a b -> Util.power a b) x
      )
and eval_list l op x =
    match l with
      [] -> 0.
    | h :: t -> op (eval_expr h x) (eval_list t op x)